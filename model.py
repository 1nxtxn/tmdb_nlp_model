...
# 🔧 Fix index alignment before vectorizing
...
def show_example_matches(user_text):
    ...
    print("\n🔍 Based on your input, here are some movie matches:")
...
def recommend_movies(user_text, user_genre, start_year, end_year, movie_region="Any"):
    ...
    if subset.empty:
        print("⚠️ No movies with revenue data match your filters.")
...
# === Run & display ===
user_text = input("💬 What kind of movie are you in the mood for?\n")
show_example_matches(user_text)
print_genre_suggestions()
print_region_suggestions()

genre = input("\n🎞️ Preferred genre? (Type 'Any' to skip):\n") or "Any"
region = input("\n🌍 Preferred region or country? (Type 'Any' to skip):\n") or "Any"
try:
    start_year = int(input("📅 Start year? (e.g. 1990):\n") or 1900)
    end_year   = int(input("📅 End year? (e.g. 2025):\n") or 2025)
except ValueError:
